 ...
 *
 * Vendored from https://github.com/binji/wasm-clang (live demo:
 * https://binji.github.io/wasm-clang/), self-hosted here (see
 * vendor/wasm-clang/README.md) instead of depending on Wasmer's registry --
 * see WasmToolchain.js for why. Three small, clearly-marked additions on
 * top of the upstream file, all inside this same closure since MemFS/App
 * aren't exported on their own: compile() gains `lang`/`includeDirs`
 * options (upstream hardcodes `-x c++`, this app compiles plain C, and
 * needs -I flags multi-file projects use); a new linkMulti() method (
 * upstream's link() only accepts one object file); and an `API` export at
 * the very end so a plain ES module (WasmToolchain.js) can reach it --
 * upstream only ever loads this via importScripts() inside its own Worker,
 * where the top-level `const API` binding is already in scope.
 */
...
class API {
  ...
  // `lang`/`includeDirs` are this app's own additions -- see this file's
  // top-of-file comment. `lang` defaults to upstream's original 'c++' so
  // compileLinkRun() below (unused by this app, kept for parity with
  // upstream) still behaves the same way it always did.
  async compile(options) {
    ...
    const includeFlags = (options.includeDirs || []).flatMap(d => ['-I', d]);

    await this.ready;
    this.memfs.addFile(input, contents);
    const clang = await this.getModule(this.clangFilename);
    return await this.run(clang, 'clang', '-cc1', '-emit-obj',
                          ...this.clangCommonArgs, ...includeFlags, '-O2',
                          '-o', obj, '-x', lang, input);
  }
  ...
  async link(obj, wasm) {
    return await this.linkMulti([obj], wasm);
  }

  // This app's own addition (see top-of-file comment) -- upstream's link()
  // only ever links a single object file, since its demo only ever
  // compiles one. `objs` here is an array so a real multi-file project
  // (Playground, an exercise with helper .c files) links correctly.
  async linkMulti(objs, wasm) {
    ...
    return await this.run(
        ...
        '-z', `stack-size=${stackSize}`, `-L${libdir}`, crt1, ...objs, '-lc',
  ...
  // This app's own addition (see top-of-file comment) -- same as run()
  // above but without the "> args...\n" hostLog line or the trailing
  // hostWrite('\n'). Used specifically for running the student's own
  // compiled program: run()'s own framing text was silently leaking into
  // what WasmToolchain.js captures as "the program's stdout", breaking
  // Learn mode's expectedStdout comparison (a real test that printed the
  // exact right thing was still coming back with a leading "> out.wasm"
  // line, so it never matched).
  async runQuiet(module, ...args) {
    const app = new App(module, this.memfs, ...args);
...
// This app's own addition (see top-of-file comment) -- upstream never
// needed this since it only ever loads this file via importScripts()
// inside its own Worker, where the top-level `const API` binding above is
// already directly in scope for its sibling script. Loaded as a plain
// <script> from an ES module instead, so it needs an explicit handoff.
(typeof window !== 'undefined' ? window : self).WasmClangAPI = API;
