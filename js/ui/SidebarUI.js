...
const CHEVRON_ICON = `<svg class="w-3 h-3 flex-shrink-0 transition-transform duration-150" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 20 20"><path stroke-linecap="round" stroke-linejoin="round" d="M7 5l6 5-6 5"></path></svg>`;
...
export class SidebarUI {
    ...
    initEventListeners() {
        ...
        if (this.sidebar) {
            if (window.innerWidth >= 1024) {
                this.sidebar.classList.remove('w-0', 'border-0', '-translate-x-full');
                this.sidebar.classList.add('w-64', 'border-r', 'lg:border-2');
            } else {
    ...
    toggleMobileSidebar(show) {
        ...
        if (show) {
            this.sidebar.classList.remove('w-0', 'border-0', 'opacity-0', 'pointer-events-none');
            this.sidebar.classList.add('w-64', 'border-r', 'lg:border-2');
            if (window.innerWidth < 1024 && this.sidebarOverlay) {
                this.sidebarOverlay.classList.remove('hidden');
            }
        } else {
            this.sidebar.classList.add('w-0', 'border-0', 'opacity-0', 'pointer-events-none');
            this.sidebar.classList.remove('w-64', 'border-r', 'lg:border-2');
    ...
    renderFolderRow(node, depth, currentFile, isDark, fragment) {
        ...
        row.className = "group p-2 cursor-pointer rounded-sm flex items-center justify-between gap-2 text-[10px] font-bold uppercase tracking-[0.15em] transition-colors duration-150";
        row.style.paddingLeft = `${8 + depth * INDENT_PX}px`;
        row.style.paddingRight = '6px';
        row.style.color = "var(--header-text)";
        row.onmouseenter = () => { row.style.backgroundColor = "rgba(100,100,100,0.08)"; };
        ...
        folderIcon.className = "text-indigo-400 flex-shrink-0";
        ...
        menuBtn.className = "p-1 rounded hover:bg-white/10 transition-colors normal-case";

        const dropdown = document.createElement('div');
        dropdown.className = "hidden absolute right-0 top-8 w-32 bg-[var(--sidebar-bg)] border border-[var(--border-color)] rounded shadow-2xl z-50 py-1 backdrop-blur-md file-menu normal-case";
    ...
    renderFileRow(node, depth, currentFile, isDark) {
        ...
        div.className = `group cursor-pointer rounded-sm flex justify-between items-center text-xs transition-all duration-200 border-l-2 py-2`;
        ...
        nameContainer.innerHTML = `
            <svg class="w-3.5 h-3.5 flex-shrink-0 ${iconColor} ${isActive ? 'animate-pulse' : ''}" fill="currentColor" viewBox="0 0 20 20">${FILE_ICON_PATH}</svg>
        ...
        const dropdown = document.createElement('div');
        dropdown.className = "hidden absolute right-0 top-8 w-32 bg-[var(--sidebar-bg)] border border-[var(--border-color)] rounded shadow-2xl z-50 py-1 backdrop-blur-md file-menu";
        ...
        renameOpt.onclick = (e) => {
            ...
            this.fsBloc.renameFile(filename, newFilename, this.fsBloc.state.currentFile === filename ? currentContent : undefined);
        };
        dropdown.appendChild(renameOpt);
    ...
    updateProjectBadge(type, name) {
        ...
        if (type === 'cloud') {
            ...
            this.projectBadge.className = 'mx-3 mt-3 px-2 py-1 bg-cyan-900/30 border border-cyan-500/30 rounded text-[9px] text-cyan-400 font-bold uppercase text-center tracking-wider truncate';
        } else if (type === 'example') {
            this.projectBadge.innerText = 'Example: ' + name;
            this.projectBadge.className = 'mx-3 mt-3 px-2 py-1 bg-purple-900/30 border border-purple-500/30 rounded text-[9px] text-purple-400 font-bold uppercase text-center tracking-wider truncate';
        } else {
            this.projectBadge.innerText = 'Project: Scratchpad';
            this.projectBadge.className = 'mx-3 mt-3 px-2 py-1 bg-[var(--track-bg)] border border-[var(--track-border)] rounded text-[9px] text-[var(--sidebar-text)] font-bold uppercase text-center tracking-wider truncate';
        }
    }
}
